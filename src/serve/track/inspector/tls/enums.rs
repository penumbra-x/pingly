...
impl_u16_is_grease!(TlsVersion);

enum_builder! {
    /// The `Cipher` TLS protocol enum.  Values in this enum are taken
    /// from the various RFCs covering TLS, and are listed by IANA.
    /// The `Unknown` item is used when processing unrecognised ordinals.
    @U16
    pub enum Cipher {
...
enum_builder! {
    ...
    pub enum SignatureAlgorithm {
        ...
        SHA384_DSA => 0x0502,
        // also labeled as ecdsa_secp384r1_sha384
        ...
        // also known as RSA_PSS_RSAE_SHA384
        RSA_PSS_SHA384 => 0x0805,
        // also known as RSA_PSS_RSAE_SHA512
...
impl_u16_is_grease!(SignatureAlgorithm);
...
impl ApplicationProtocol {
    pub fn decode(r: &mut impl std::io::Read) -> std::io::Result<ApplicationProtocol> {
        let mut length = [0];
...
impl From<tokio_rustls::rustls::CipherSuite> for Cipher {
    fn from(value: tokio_rustls::rustls::CipherSuite) -> Self {
        let n: u16 = value.into();
        Cipher::from(n)
    }
}

impl From<tokio_rustls::rustls::SignatureScheme> for SignatureAlgorithm {
    fn from(value: tokio_rustls::rustls::SignatureScheme) -> Self {
        let n: u16 = value.into();
        SignatureAlgorithm::from(n)
    }
}

impl From<tokio_rustls::rustls::ProtocolVersion> for TlsVersion {
    fn from(value: tokio_rustls::rustls::ProtocolVersion) -> Self {
        let n: u16 = value.into();
        TlsVersion::from(n)
    }
}
...
mod tests {
    ...
    fn test_application_protocol_decode_wire_format_multiple() {
        ...
        assert_eq!(
            ...
            ApplicationProtocol::decode(&mut r).unwrap()
        );
